...
contract xWinSystemExecutor is OwnableUpgradeable {
   ...
   function runSystemDeposit(address[] calldata _addresses) external onlyExecutor {
        
        for(uint256 i = 0; i < _addresses.length; i++) {
            IxWinSingleAssetInterface _obj = IxWinSingleAssetInterface(_addresses[i]);
    ...
    function runSystemReinvestReclaim(address[] calldata _addresses) external onlyExecutor {
    ...
    function runSystemReTrade(address[] calldata _addresses) external onlyExecutor {
    ...
    function runSystemAddTokenPath(
        ...
        for(uint256 i = 0; i < swapInfos.length; i++) {
            ...
            _obj.addTokenPath(swapInfos[i].router, swapInfos[i].fromToken, swapInfos[i].toToken, swapInfos[i].path, swapInfos[i].slippage);
        }
    }
