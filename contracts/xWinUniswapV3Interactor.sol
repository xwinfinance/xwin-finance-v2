...
contract xWinUniSwapV3Engine is xWinStrategyInteractor {
    ...
    function swapTokenToToken(uint _amount, address _fromToken, address _toToken) public payable returns (uint){
        ...
        require(_fromToken != address(0) && _toToken != address(0), "xWinSwap, input token is empty");
    ...
    function _xWinStratSwap(uint256 _amount, address _fromToken, address _toToken) internal returns(bool, uint) {
        if (isxWinStrategy(_fromToken)) {
            ...
            if (baseToken != _toToken) {
                uint newAmount = swapWithUni(output, baseToken, _toToken, address(this));
                output = newAmount;
            }
            ...
            if (baseToken != _fromToken) {
                uint newAmount = swapWithUni(_amount, _fromToken, baseToken, address(this));
    ...
    function swapWithUni(uint _amount, address _fromToken, address _toToken, address _recipient) internal returns (uint) {
        ...
        if (paths.isMultihop) {
            ...
            require(paths.poolFee != 0, "swap path not registered for token pair");
    ...
    function isValidCaller(address sender) private view {
        require(xWinStrategies[sender].baseToken > address(0), "xWinSwap: caller not registered");
    }
