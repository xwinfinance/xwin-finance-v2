...
abstract contract xWinStrategyWithFee is xWinStrategy {
    ...
    function collectPerformanceFee() external virtual {
        ...
        if (watermarkUnitprice < unitPrice) {
            ...
            uint totalProfit = ((unitPrice - watermarkUnitprice) * totalUnits) /
                1e18;
    ...
    function performanceWithdraw(
        ...
        if (
            unitPrice <= watermarkUnitprice || waivedPerformanceFees[msg.sender]
        ) {
        ...
        uint256 profit = ((unitPrice - watermarkUnitprice) *
            _withdrawUnits *
            blockDiff) /
            collectionPeriod /
            1e18;
        ...
        if (lockingAddress != address(0)) {
            uint256 discount = ILockedStake(lockingAddress).getFavor(
                msg.sender
            );
    ...
    function _getMintQty(
        ...
        uint256 totalFundB4 = totalFundAfter > _depositAmt
            ? totalFundAfter - _depositAmt
            : 0;
    ...
    function _getNewFundUnits(
        ...
        uint256 totalUnitAfter = (totalValueAfter * getFundTotalSupply()) /
            totalFundB4;
    ...
    function setPerformanceCollectionPeriod(
        uint256 _newPeriod
    ) external onlyOwner {
