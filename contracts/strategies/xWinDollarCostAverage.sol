...
// contract xWinDollarCostAverage is xWinStrategyWithFee {
...
//         __xWinStrategyWithFee_init("xWIN Dollar Average Vault", "xDCA", _baseToken, _USDTokenAddr, _managerFee, _performanceFee, _collectionPeriod, _managerAddr);
...
//         // Infinite approve
//         IERC20Upgradeable(_baseToken).safeApprove(stableCoinStaking_, type(uint).max);
//         targetToken.safeApprove(targetTokenStaking_, type(uint).max);
//         IERC20Upgradeable(_baseToken).safeApprove(address(swapEngine), type(uint).max);
//         targetToken.safeApprove(address(swapEngine), type(uint).max);
...
//         _stableCoinStaking.withdraw(amtToSwap);
//         // swap to targetToken
//         uint stableToswap = IERC20Upgradeable(baseToken).balanceOf(address(this));
//         uint btcOutput = swapEngine.swapTokenToToken(stableToswap, baseToken, address(targetToken));
...
//         uint baseTokenBalInStaking = _stableCoinStaking.getUserBalance(address(this));
...
//         }
//         // watermark up:150
//         // any deposit where unitprice > watermark seems unfair, instantly gets charged with performance fee
//         // _processPerformOnWithdraw(totalRefund); // this does not change watermark
...
//     // update properties
//     function updateProperties(uint _maxPerSwap, uint _swapDuration, uint _reinvestDuration) public onlyOwner {
...
//         }
//         if (targetToken.allowance(address(this), address(swapEngine)) == 0) {
//             targetToken.safeApprove(address(swapEngine), type(uint).max);
//         }
//     }
