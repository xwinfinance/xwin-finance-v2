...
contract xWinBuddyChef is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _devAddress, address _xWinPriceMaster, address _usdtToken, uint _blocksPerDay) initializer public {
        require(_devAddress != address(0), "devAddress input zero");
        require(_xWinPriceMaster != address(0), "xWinPriceMaster addr input zero");
    ...
    function add(IERC20Upgradeable _lpToken, IERC20Upgradeable _rewardToken, uint _duration, uint _totalBuddyTokens) public onlyOwner {        
        
        require(_lpToken != _rewardToken, "cannot be same token");
        _rewardToken.safeTransferFrom(msg.sender, address(this), _totalBuddyTokens);
        massUpdatePools();
    ...
    function set(uint _pid, uint _newDuration, uint _totalBuddyTokens, uint _amountToAdd) public onlyOwner {
        massUpdatePools();
        PoolInfo memory pool = poolInfo[_pid];
        pool.rewardToken.safeTransferFrom(msg.sender, address(this), _amountToAdd);
        if (pool.endPeriod > block.number) {
            uint tokensLeftover = (pool.endPeriod - block.number) * pool.tokenPerBlock;
            require(_amountToAdd + tokensLeftover >= _totalBuddyTokens, "insufficient amt to add");
        } else {
            require(_amountToAdd >= _totalBuddyTokens, "insufficient amt to add");
        }
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && pool.totalStaked != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.endPeriod);
            uint256 cakeReward = multiplier * pool.tokenPerBlock;
            accCakePerShare = accCakePerShare + cakeReward * 1e18 / pool.totalStaked;
        }
        uint rewardBal = (user.amount * accCakePerShare / 1e18) - user.rewardDebt;
        return rewardBal;
    }
    
    // Return reward multiplier over the given _from to _to block.
    function getMultiplier(uint _from, uint _to, uint _endBlock) public view returns (uint) {
    ...
    function updatePool(uint _pid) public {
        ...
        uint multiplier = getMultiplier(pool.lastRewardBlock, block.number, pool.endPeriod);
        uint cakeReward = multiplier * pool.tokenPerBlock; 
        pool.accCakePerShare = pool.accCakePerShare + (cakeReward * 1e18 / pool.totalStaked);
        pool.lastRewardBlock = block.number;
    ...
    function safeTokenTransfer(IERC20Upgradeable rewardToken, address _to, uint256 _amount) internal {
