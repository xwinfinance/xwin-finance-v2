...
contract xWinBBMA is xWinStrategyWithFee, KeeperCompatibleInterface {
    ...
    function initialize(
        ...
        __xWinStrategyWithFee_init("xWIN BBMA", "xBBMA", _baseToken, _USDTokenAddr, _managerFee, _performanceFee, _collectionPeriod, _managerAddr);
    ...
    function updatexWinPriceMaster(address _xWinPriceMaster) external onlyOwner {
        xWinPriceMaster = IxWinPriceMaster(_xWinPriceMaster);
    }
    ...
    function systemReTrade()
        ...
        require(tr.nextTrade && tr.nextTradeBlock <= block.number, "not allow to retrade yet");
     ...
     */
    function systemTrade(uint8 _tradeType)
        public
        onlyExecutor
        nonReentrant {
    ...
    function _systemTrade(uint8 _tradeType) internal {
        ...
        if(_tradeType == 0){
            ...
            tradeQueue.nextTradeBlock = stablebal != 0 ? block.number + tradeCycle : block.number;
        ...
        }else if(_tradeType == 1)
        {
            ...
            uint stableOutput = swapEngine.swapTokenToToken(amtToSwap, address(targetToken), baseToken);
            targetbal = targetToken.balanceOf(address(this));
            
            tradeQueue.nextTrade = targetbal != 0;
            tradeQueue.nextTradeBlock = targetbal != 0 ? block.number + tradeCycle : block.number;
            tradeQueue.tradeType = _tradeType;
            stopLossPrice = 0;
            takeProfitPrice = 0;
            emitEvent.FundEvent("systemCloseBuy", address(this), msg.sender, unitprice, amtToSwap, stableOutput);
        }
    }
