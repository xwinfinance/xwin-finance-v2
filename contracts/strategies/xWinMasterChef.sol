...
contract xWinMasterChef is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    function initialize(string calldata name, string calldata symbol, address _usdtToken, uint256 _blocksPerDay) initializer external {
    ...
    function farmTokenByAdmin() external onlyOwner {
        IERC20Upgradeable(address(this)).safeIncreaseAllowance(address(_xwinDefi), totalSupply());
    ...
    function updateSmartContract(address _xwinDefiaddr, address _xWinPriceMaster) external onlyOwner {
    ...
    function add(uint256 _allocPoint, IERC20Upgradeable _lpToken, uint256 _duration) external onlyOwner {
        massUpdatePools();
    ...
    function set(uint256 _pid, uint256 _allocPoint, uint256 _newDuration) external onlyOwner {
    ...
    function pendingRewards(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accCakePerShare = accCakePerShare + (cakeReward * 1e18 / totalAllocPoint / lpSupply);
        }
        uint256 pendingBeforeFee = (user.amount * accCakePerShare / 1e18) - user.rewardDebt;
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 rewardBlock = pool.endPeriod > block.number ? block.number : pool.endPeriod;
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, rewardBlock);
        uint256 cakeReward = multiplier * xwinPerBlock * pool.allocPoint;

        //harvest from xwin farm
        _xwinDefi.DepositFarm(xwinpid, 0);
        pool.accCakePerShare = pool.accCakePerShare + (cakeReward * 1e18 / totalAllocPoint / lpSupply);
        pool.lastRewardBlock = block.number;
    ...
    function deposit(uint256 _pid, uint256 _amount) external nonReentrant returns(uint256) {
    ...
    function _deposit(uint256 _pid, uint256 _amount) internal returns(uint256 rewardAmount) {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount * pool.accCakePerShare / 1e18 - user.rewardDebt;
            if(pending > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) external nonReentrant returns (uint256 rewardAmount) {
    ...
    function harvest(uint256 _pid) external nonReentrant returns(uint256){
        ...
        userInfo[_pid][msg.sender].rewardDebt = user.amount * pool.accCakePerShare / 1e18;
        return pending;
    }
    ...
    function getAPR(uint _pid) external view 
        returns (uint256 apr){
        
        PoolInfo memory pool = poolInfo[_pid];
        uint lpBal = pool.totalDeposit;
        if(address(pool.lpToken) == address(0) || lpBal == 0) return 0;
        uint xwinPrice = IxWinPriceMaster(xWinPriceMaster).getPrice(address(rewardsToken), usdtToken);
        uint tokenPrice = IxWinPriceMaster(xWinPriceMaster).getPrice(address(pool.lpToken), usdtToken);
    ...
    function getPoolUserRewardPerBlock(uint _pid, address _user) external view returns (uint256) {
        PoolInfo memory pool = poolInfo[_pid];
        UserInfo memory user = userInfo[_pid][_user];
        uint256 userRatio = user.amount * 1e18 / pool.totalDeposit;
        uint256 rewardPerBlockUser = xwinPerBlock * pool.allocPoint * userRatio / totalAllocPoint;
