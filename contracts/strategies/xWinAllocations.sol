...
contract xWinAllocations is xWinStrategyWithFee {
    ...
    function _deposit(uint256 _amount, uint32 _slippage) internal returns (uint256) {
        ...
        if(!_isContract(msg.sender)){
            emitEvent.FundEvent("deposit", address(this), msg.sender, _convertTo18(unitPrice, baseToken), _amount, currentShares);
        }
    ...
    function _withdraw(uint256 _shares, uint32 _slippage) internal returns (uint256){
        ...
        if (totalOutput > 0) IERC20Upgradeable(baseToken).safeTransfer(msg.sender, totalOutput);
        
        if(!_isContract(msg.sender)){
            emitEvent.FundEvent("withdraw", address(this), msg.sender, unitP, totalOutput, _shares);
        }
        return totalOutput;
    }
    ...
    function createTargetNames(address[] calldata _toAddr,  uint256[] calldata _targets) onlyExecutor public {
    ...
    function Rebalance(
        ...
        emitEvent.FundEvent("rebalance", address(this), msg.sender, getUnitPrice(), 0, 0);
    }
    ...
    function getUnitPrice() public override view returns (uint256) {        
        return getFundTotalSupply() == 0 ? 1e18 : _convertTo18(_getVaultValues() *  1e18 / getFundTotalSupply(), baseToken);
    }

    function _getUnitPrice() internal override view returns (uint256) {        
        return getFundTotalSupply() == 0 ? getDecimals(baseToken) : _getVaultValues() *  1e18 / getFundTotalSupply();
    }

    function getUnitPriceInUSD() public override view returns (uint256) {        
        return getFundTotalSupply() == 0 ? 1e18 : _convertTo18(_getVaultValuesInUSD() * 1e18 / getFundTotalSupply(), stablecoinUSDAddr);
    }
