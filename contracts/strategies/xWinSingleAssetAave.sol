...
contract xWinSingleAssetAave is xWinStrategyWithFee {
    ...
    function getSupplyRate() public view returns (uint256) {
        (, , , , , uint256 liquidityRate, , , , , , ) = aavePoolDataProvider
    ...
    function _getMintQty(
        uint256 _unitPrice
    ) internal view override returns (uint256 mintQty) {
    ...
    function _withdraw(
        ...
        uint totalAaveAmount = IERC20Upgradeable(targetToken).balanceOf(
            address(this)
        );
        uint withdrawAaveAmt = (redeemratio * totalAaveAmount) / 1e18;
        withdrawAaveAmt = totalAaveAmount < withdrawAaveAmt
            ? totalAaveAmount
            : withdrawAaveAmt;
        if (withdrawAaveAmt > 0)
            aavePool.withdraw(baseToken, withdrawAaveAmt, address(this));
        uint baseBalAfter = IERC20Upgradeable(baseToken).balanceOf(
            address(this)
        );
    ...
    function _getVaultValuesInUSD() internal view returns (uint256) {
        uint baseTokenUSDPrice = xWinPriceMaster.getPrice(
            baseToken,
            stablecoinUSDAddr
        );
