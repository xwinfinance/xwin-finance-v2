...
contract xWinLockedStake is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC20Upgradeable {
    ...
    function masterChefDeposit() external onlyOwner {
        ...
        IERC20Upgradeable(address(this)).safeIncreaseAllowance(address(masterChef), 1 ether);
     ...
     */
    function deposit(uint256 _amount, uint8 _duration) nonReentrant external {
        ...
        if (user.shares > 0) { // this deposit is a renew
            uint256 newEndTime = block.timestamp + convertWeeksToTimestamp(_duration);
            require(newEndTime > user.endPeriodTimestamp, "Invalid duration input");

            uint256 oldLockedShares = user.lockedShares;
            uint256 rewardAmount = (oldLockedShares * accXWINperLockedShare / 1e18) - user.rewardDebt;
            ...
            user.rewardDebt = user.lockedShares * accXWINperLockedShare / 1e18;
            user.xWinFavor = calculateFavor(user.amount, user.startTimestamp, user.endPeriodTimestamp);
            totalShares += currentShares;
            totalLockedShares = totalLockedShares - oldLockedShares + user.lockedShares;

        } else { //this is a new deposit
            user.shares = currentShares;
            user.amount = _amount;
            user.startTimestamp = block.timestamp;
            user.lockedShares = shareMultiplier(_amount, _duration);
            user.rewardDebt = user.lockedShares * accXWINperLockedShare / 1e18;
            user.endPeriodTimestamp = block.timestamp + convertWeeksToTimestamp(_duration);
            user.xWinFavor = calculateFavor(user.amount, user.startTimestamp, user.endPeriodTimestamp);
            totalShares += currentShares;
    ...
    function harvest() nonReentrant public
    {
        _harvest();
    }
    ...
    function withdraw() nonReentrant external{
        ...
        require(user.endPeriodTimestamp <= block.timestamp, "tokens still locked");
        _harvest();
    ...
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            IERC20Upgradeable(token).safeIncreaseAllowance(address(masterChef), bal);
     ...
     */
    function setFees(uint256 _callFee, uint256 _performanceFee) external onlyOwner {
    ...
    function migrateMasterChef(address _newMasterChef, uint256 _xwinpid, uint256 _lockpid) external onlyOwner {
        _harvest();
        ...
        IERC20Upgradeable(token).safeIncreaseAllowance(_newMasterChef, totalAmt);
        IERC20Upgradeable(address(this)).safeIncreaseAllowance(_newMasterChef, 1 ether);
        xwinpid = _xwinpid;
        lockpid = _lockpid;
    ...
    function _doWithdraw(UserInfo memory user) internal returns (uint256) {
        // auto XWIN portion
        uint256 currentAmount = totalXWINBalance() * user.shares / totalShares;
        
        // bonus portion
        uint256 bonusAmount = (user.lockedShares * accXWINperLockedShare / 1e18) - user.rewardDebt;
    ...
    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : totalXWINBalance() * 1e18 / totalShares;
    }
    ...
    function totalXWINBalance() public view returns (uint256) {
        ...
        return token.balanceOf(address(this)) + depositAmount + pending - lockedRewardsVault;
    }
    ...
    function getUserPosition(address _user) public view returns (uint256 rewardAmount, uint256 xwinAmount) {
        ...
        uint pendingFromBonusPool = masterChef.pendingRewards(lockpid, address(this));
        uint256 tempaccXWINperLockedShare = accXWINperLockedShare + (1e18 * pendingFromBonusPool) / totalLockedShares;
        rewardAmount = (user.lockedShares * tempaccXWINperLockedShare / 1e18) - user.rewardDebt;
    ...
    function calculateFavor(uint256 amount, uint256 startTimestamp, uint256 endTimeStamp) private pure returns (uint256 favor) {
    ...
    function getUserCompoundAPYrate(address _user) external view returns (uint256 estimatedDailyRate) {
        ...
        uint emmission = masterChef.getPoolUserRewardPerBlock(xwinpid, address(this));
        uint256 estimatedDailyReward = emmission * blocksPerDay * user.shares;
        estimatedDailyRate = estimatedDailyReward * 1e18 / totalShares / user.amount;
    ...
    function getUserLockingBonusAPR(address _user) external view returns (uint256 apr) {
        ...
        UserInfo memory user = userInfo[_user];
        if(user.amount == 0) return 0;
        uint256 emmission = masterChef.getPoolUserRewardPerBlock(lockpid, address(this));
        uint256 estimatedReward = emmission * 365 * blocksPerDay * user.lockedShares ;
    ...
    function getEstimatedDepositAPY(uint256 _amount, uint8 _duration) external view returns (uint256 estimatedDailyRate, uint256 estimatedBonusApr) {
        uint emmissionCompound = masterChef.getPoolUserRewardPerBlock(xwinpid, address(this));
        uint256 emmissionBonus = masterChef.getPoolUserRewardPerBlock(lockpid, address(this));
        ...
        uint256 estimatedDailyReward = emmissionCompound * blocksPerDay * newShares;
        // this is returned
        estimatedDailyRate = estimatedDailyReward * 1e18 / (totalShares + newShares) / _amount;
        // calculate in frontend (1 + (estimatedDailyRate / 1e18)) ** 365 - 1

        // Bonus Apr calculation
        uint256 newLockedShares = shareMultiplier(_amount, _duration);
        uint256 estimatedReward = emmissionBonus * 365 * blocksPerDay * newLockedShares;
        // this is returned
        estimatedBonusApr = estimatedReward * 10000 / (totalLockedShares + newLockedShares) / _amount;
    }   
