...
contract xWinSingleAssetAlpaca is xWinStrategyWithFee {
    ...
    function _withdraw(
        ...
        uint totalOlaAmount = IERC20Upgradeable(address(_Alpaca)).balanceOf(
            address(this)
        );
    ...
    function _getVaultValuesInUSD() internal view returns (uint256) {
        ...
        uint balanceStakingToken = IERC20Upgradeable(address(_Alpaca))
    ...
    function emergencyUnWindPosition()
        ...
        onlyOwner
    {
        uint balanceToken = IERC20Upgradeable(address(_Alpaca)).balanceOf(
            address(this)
        );
