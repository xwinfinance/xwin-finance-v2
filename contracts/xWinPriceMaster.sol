    ...
    */
    ...
    function getPrice(
        ...
        if (map.source == 0) {
            ...
            require(
                map.source != 0,
                "PriceMaster: unsupported input addresses"
            );
        }

        if (map.source == 1) {
            // chainlink
            AggregatorV3Interface chainLinkFeedFrom = AggregatorV3Interface(
                chainLinkUSDpair[_from]
            );
            AggregatorV3Interface chainLinkFeedTo = AggregatorV3Interface(
                chainLinkUSDpair[_to]
            );

            uint256 chainlinkDecimalsFrom = 10 ** chainLinkFeedFrom.decimals();
            (
                ,
                /*uint80 roundID*/ int256 priceFrom /*uint startedAt*/ /*uint256 updatedAt*/ /*uint80 answeredInRound*/,
                ,
                ,

            ) = chainLinkFeedFrom.latestRoundData();
            int192 min1 = AccessControlledOffchainAggregatorInterface(
                chainLinkFeedFrom.aggregator()
            ).minAnswer();
            int192 max1 = AccessControlledOffchainAggregatorInterface(
                chainLinkFeedFrom.aggregator()
            ).maxAnswer();
            uint256 chainlinkDecimalsTo = 10 ** chainLinkFeedTo.decimals();
            (
                ,
                /*uint80 roundID*/ int256 priceTo /*uint startedAt*/ /*uint256 updatedAt*/ /*uint80 answeredInRound*/,
                ,
                ,

            ) = chainLinkFeedTo.latestRoundData();
            int192 min2 = AccessControlledOffchainAggregatorInterface(
                chainLinkFeedTo.aggregator()
            ).minAnswer();
            int192 max2 = AccessControlledOffchainAggregatorInterface(
                chainLinkFeedTo.aggregator()
            ).maxAnswer();
            require(
                priceFrom.toInt192() >= min1 &&
                    priceFrom.toInt192() <= max1 &&
                    priceTo.toInt192() >= min2 &&
                    priceTo.toInt192() <= max2,
                "Chainlink price outside operation range"
            );
            uint256 rateTo = chainlinkDecimalsTo * chainlinkDecimalsTo;
            rate =
                (priceFrom.toUint256() * rateTo * decimalPlaces) /
                priceTo.toUint256() /
                chainlinkDecimalsFrom /
                chainlinkDecimalsTo;
            ...
            (
                ,
                /*uint80 roundID*/ int256 price /*uint startedAt*/ /*uint256 updatedAt*/ /*uint80 answeredInRound*/,
                ,
                ,

            ) = chainLinkFeed.latestRoundData();
            int192 min = AccessControlledOffchainAggregatorInterface(
                chainLinkFeed.aggregator()
            ).minAnswer();
            int192 max = AccessControlledOffchainAggregatorInterface(
                chainLinkFeed.aggregator()
            ).maxAnswer();
            require(
                price.toInt192() >= min && price.toInt192() <= max,
                "Chainlink price outside operation range"
            );
    ...
    function _getLPPrice(
        ...
        uint8 d3 = (d0 + d1) / 2;
        return
            (2 * reservesSqrt * rateSqrt * 10 ** d2) / totalSupply / 10 ** d3;
    }
        ...
        if (_source == 1) {
            require(
                chainLinkUSDpair[_from] != address(0) &&
                    chainLinkUSDpair[_to] != address(0),
                "token/usd pair missing"
            );
        }
        if (_source == 2) {
    ...
    function addChainlinkUSDPrice(
        address _token,
        address _chainLinkAddr
    ) external onlyExecutor {
        require(
            _token != address(0) && _chainLinkAddr != address(0),
            "empty inputs"
        );
