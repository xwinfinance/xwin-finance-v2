...
describe("TokenAlpha", function () {
  describe("Core", function () {
    it("xUSDT-xBTC", async function () {
      const { owner, accounts, xWinTokenAlpha, xUSDT, USDT, BTCB } = await loadFixture(
        xWinFixture
      );
      ...
      expectAlmostEquals(await xWinTokenAlpha.getUnitPrice(), ethers.parseEther("1"));
      expectAlmostEquals(await xWinTokenAlpha.getVaultValues(), ethers.parseEther("0"));
      expectAlmostEquals(
      ...
      expectAlmostEquals(await xWinTokenAlpha.getTargetValues(), ethers.parseEther("100"));
      expectAlmostEquals(await xWinTokenAlpha.getBaseValues(), ethers.parseEther("0"));
  


      await xWinTokenAlpha.connect(accounts[0]).deposit(defaultAmount);

      expectAlmostEquals(await xWinTokenAlpha.getUnitPrice(), ethers.parseEther("1.01"));
      expectAlmostEquals(
        await xWinTokenAlpha.getVaultValues(),
      ...
      await network.provider.send("hardhat_mine", ["0x7080"]);
      await xWinTokenAlpha.systemDeposit();
      expectAlmostEquals(await xWinTokenAlpha.getStableValues(), ethers.parseEther("20000"));
      expectAlmostEquals(await xWinTokenAlpha.getTargetValues(), ethers.parseEther("300"));
      expectAlmostEquals(await xWinTokenAlpha.getBaseValues(), ethers.parseEther("0"));

      await xWinTokenAlpha.withdraw(await xWinTokenAlpha.balanceOf(await owner.getAddress()));

      expectAlmostEquals(await xWinTokenAlpha.getUnitPrice(), ethers.parseEther("1.02"));
      expectAlmostEquals(
        await xWinTokenAlpha.getVaultValues(),
        defaultAmount
      );
